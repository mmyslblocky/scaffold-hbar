...
abstract contract HederaTokenService {
    ...
    int256 constant SUCCESS_CODE = 22;
    ...
    /// If the provided token is not found, the transaction will resolve to INVALID_TOKEN_REF.
    /// If the provided token has been deleted, the transaction will resolve to TOKEN_WAS_DELETED.
    /// If an association between the provided account and the token already exists, the
    /// transaction will resolve to TOKEN_ALREADY_ASSOCIATED_TO_ACCOUNT.
    /// On success, the association between the provided account and the token is made and the account is ready to interact with the token.
