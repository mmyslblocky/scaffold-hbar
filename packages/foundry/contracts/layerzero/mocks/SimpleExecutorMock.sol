...
contract SimpleExecutorMock is IWorker, AccessControl, ReentrancyGuard, IExecutor, EndpointV2View {
    ...
    function nativeDropAndExecute302(
        ...
        uint256 spent = _nativeDrop(
            _executionParams.origin, localEidV2, _executionParams.receiver, _nativeDropParams, _nativeDropGasLimit
        );
     ...
     * @dev Virtual function - can be overridden for custom fee logic
     */
    function assignJob(address _sender, bytes calldata _options)
        external
        virtual
        onlyRole(MESSAGE_LIB_ROLE)
        onlyAcl(_sender)
     ...
     * @dev Virtual function - can be overridden for custom fee logic
     */
    function getFee(address _sender, bytes calldata _options)
        external
        view
        virtual
        onlyAcl(_sender)
